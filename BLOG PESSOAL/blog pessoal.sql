...
CREATE TABLE IF NOT EXISTS `blogpessoal`.`tb_postagens` (
  ...
  `tb_temas_descricao` VARCHAR(255) NOT NULL,
  `tb_usuarios_id` BIGINT NOT NULL,
  PRIMARY KEY (`id`, `tb_postagens_id`),
  INDEX `fk_tb_postagens_tb_temas_idx` (`tb_temas_id` ASC, `tb_temas_descricao` ASC) VISIBLE,
  INDEX `fk_tb_postagens_tb_usuarios1_idx` (`tb_usuarios_id` ASC) VISIBLE,
  CONSTRAINT `fk_tb_postagens_tb_temas`
    FOREIGN KEY (`tb_temas_id` , `tb_temas_descricao`)
    REFERENCES `blogpessoal`.`tb_temas` (`id` , `descricao`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
